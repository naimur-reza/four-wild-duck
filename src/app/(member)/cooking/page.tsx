...
type MemberItem = Awaited<
  ReturnType<typeof prisma.messMember.findMany>
>[number] & {
  profile: { name: string };
};

type CookingEntryItem = Awaited<
  ReturnType<typeof prisma.cookingEntry.findMany>
>[number] & {
  member: { id: string; profile: { name: string } };
  ratings: { rating: number }[];
};

type UnavailableItem = Awaited<
  ReturnType<typeof prisma.cookingUnavailable.findMany>
>[number] & {
...
export default async function CookingPage({
  ...
  searchParams?: Promise<{ cookingStatus?: string }>;
}) {
  const params = await searchParams;
  const message = params?.cookingStatus
    ? statusMessages[params.cookingStatus]
    : undefined;
  ...
  try {
    const [entryRows, unavailableRows, completedRows, todayRows] =
      await Promise.all([
  ...
  for (const item of monthlyCompleted) {
    countByMember.set(
      item.memberId,
      (countByMember.get(item.memberId) || 0) + 1,
    );
  }

  const todayNames = todayEntries
    .map((entry) => entry.member.profile.name)
    .join(", ");
      ...
      {!cookingTablesReady ? (
        <SectionCard className="mb-3 border-amber-200 bg-amber-50 p-4 sm:p-5">
          ...
          <h3 className="mt-2 text-xl font-black text-slate-950">
            Cooking database tables are not ready
          </h3>
          <p className="mt-2 text-sm font-semibold text-amber-800">
            Run{" "}
            <code className="rounded bg-white px-1 py-0.5">pnpm db:push</code>{" "}
            once for the production Neon database, then redeploy/reload. No
            reset needed.
          </p>
        </SectionCard>
      ) : null}
      ...
      <div className="grid gap-3 lg:grid-cols-[0.8fr_1.2fr] lg:gap-4">
        <SectionCard className="p-4 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-teal-50 px-3 py-1 text-[10px] font-black text-teal-700 ring-1 ring-teal-100">
              {monthlyCompleted.length} total
            </span>
          </div>
              ...
              >
                {canManage ? (
                  <select
                    ...
                    required
                    disabled={!cookingTablesReady}
                    className="col-span-2 rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm font-semibold outline-none focus:border-teal-500 disabled:opacity-60"
                  >
                    {members.map((member) => (
                ...
                <input
                  ...
                  disabled={!cookingTablesReady}
                  className="rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-3 text-sm font-semibold outline-none focus:border-teal-500 disabled:opacity-60"
                />
                <select
                  name="status"
                ...
                <input
                  name="comment"
                  placeholder="mach, murgi, vat"
                  disabled={!cookingTablesReady}
                  className="col-span-2 rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm font-semibold outline-none focus:border-teal-500 disabled:opacity-60"
                />
                <SubmitButton
                  pendingText="Saving..."
                  className="col-span-2 rounded-2xl bg-teal-700 px-4 py-3 text-sm font-black text-white disabled:opacity-60"
                >
                  Save
                </SubmitButton>
              </form>
              ...
              <details>
                ...
                <form action={addUnavailableDate} className="mt-4 grid gap-2">
                  <select
                    ...
                    required
                    disabled={!cookingTablesReady}
                    className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm font-semibold outline-none focus:border-teal-500 disabled:opacity-60"
                  >
                  ...
                  {unavailable.map((item) => (
                    ...
                    >
                      ...
                      <form action={deleteUnavailableDate}>
                        ...
                        <SubmitButton
                          pendingText="..."
                          className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 ring-1 ring-rose-100"
                        >
        ...
        <SectionCard className="p-4 sm:p-6">
          ...
          <div className="mt-3 space-y-2 sm:mt-4 sm:space-y-3">
            {entries.length ? (
              entries.map((entry) => {
                const canEditEntry =
                  canManage || entry.memberId === membership.id;
                const avg = averageRating(entry.ratings);
                return (
                  <details
                    key={entry.id}
                    className="rounded-2xl border border-slate-100 bg-slate-50/80 p-3 open:bg-white"
                  >
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-black">
                          {entry.member.profile.name}
                        </p>
                        <p className="truncate text-xs font-bold text-slate-400">
                          {prettyDate(entry.date)} •{" "}
                          {entry.comment || "No menu"}
                        </p>
                      </div>
                      ...
                      <div className="mb-3 flex flex-wrap items-center gap-1.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <form key={star} action={rateCookingEntry}>
                            ...
                            <SubmitButton
                              pendingText="..."
                              className="rounded-full bg-amber-50 px-2.5 py-1.5 text-xs font-black text-amber-700 ring-1 ring-amber-100 hover:bg-amber-100"
                            >
                              {star}★
                            </SubmitButton>
                          </form>
                        ...
                        <>
                          ...
                          >
                            ...
                            <select
                              ...
                              className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold outline-none focus:border-teal-500"
                            >
                              <option value="COMPLETED">Cooked</option>
                              <option value="SKIPPED">Skipped</option>
                              <option value="SWAPPED">Swapped</option>
                            ...
                            <SubmitButton
                              pendingText="Saving..."
                              className="rounded-2xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white sm:col-span-2"
                            >
                              Save
                            </SubmitButton>
                          </form>
                          <form
                            action={deleteCookingEntry}
              ...
              <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-4 text-sm font-bold text-slate-500">
                No cooking entries yet.
              </p>
            )}
          </div>
        </SectionCard>
